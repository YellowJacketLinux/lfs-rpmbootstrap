...
mv %{buildroot}%{_libdir}/libstdc++.so.%{libstdcppv} %{buildroot}/%{_lib}/
rm -f %{buildroot}%{_libdir}/libstdc++.so.6
ln -sf libstdc++.so.%{libstdcppv} %{buildroot}/%{_lib}/libstdc++.so.6
rm -f %{buildroot}%{_libdir}/libstdc++.so
ln -sf ../../%{_lib}/libstdc++.so.%{libstdcppv} %{buildroot}%{_libdir}/libstdc++.so
...
%defattr(-,root,root,-)
%attr(0755,root,root) /%{_lib}/libstdc++.so.%{libstdcppv}
/%{_lib}/libstdc++.so.6
...
%attr(0644,root,root) %{_datadir}/gdb/auto-load/usr/%{_lib}/libstdc++.so.%{libstdcppv}-gdb.py
...
%attr(0755,root,root) %{_prefix}/lib/gcc/%{triplet}/%{version}/adalib/libgnarl-14.so
%attr(0755,root,root) %{_prefix}/lib/gcc/%{triplet}/%{version}/adalib/libgnat-14.so
...
%defattr(-,root,root,-)
%attr(0755,root,root) %{_libdir}/libgdruntime.so.5.0.0
...
%files -n libgphobos
%defattr(-,root,root,-)
%attr(0755,root,root) %{_libdir}/libgphobos.so.5.0.0
...
%files -n libgo
%defattr(-,root,root,-)
%attr(0755,root,root) %{_libdir}/libgo.so.23.0.0
%{_libdir}/libgo.so.23
...
- Rebuild with make check needed after Valgrind and gdb available.
